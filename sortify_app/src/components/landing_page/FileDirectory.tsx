...
const CATEGORY_ICON_SIZE = 'w-10 h-10';
...
const isFileType = (fileName: string, extensions: string[]): boolean => {
  const lowerName = fileName.toLowerCase();
  return extensions.some(ext => lowerName.endsWith(ext));
};

// Helper function to format file count text
...
const groupFilesByCategory = (files: UploadedFile[]): Record<string, UploadedFile[]> => {
...
// Helper function to sort files by creation date
const sortFilesByDate = (files: UploadedFile[]): UploadedFile[] => {
...
export default function FileDirectory({
  ...
  const getFileIcon = (file: UploadedFile) => {
    ...
    if (type.includes('mp4') || type.includes('video') || isFileType(name, ['.mp4', '.webm', '.ogg', '.mov', '.avi'])) {
      return <Film className={`${ICON_SIZE} text-muted-foreground`} />;
    }
    if (type.includes('image') || isFileType(name, ['.jpg', '.jpeg', '.png', '.gif', '.bmp', '.webp', '.svg'])) {
      return <ImageIcon className={`${ICON_SIZE} text-muted-foreground`} />;
    }
    if (type.includes('pdf') || isFileType(name, ['.pdf'])) {
      return <FileText className={`${ICON_SIZE} text-red-500`} />;
    }
    if (type.includes('word') || type.includes('document') || isFileType(name, ['.doc', '.docx'])) {
      return <FileText className={`${ICON_SIZE} text-blue-500`} />;
    }
    if (type.includes('excel') || type.includes('spreadsheet') || isFileType(name, ['.xls', '.xlsx'])) {
      return <FileText className={`${ICON_SIZE} text-green-500`} />;
    }
    if (type.includes('powerpoint') || type.includes('presentation') || isFileType(name, ['.ppt', '.pptx'])) {
      return <FileText className={`${ICON_SIZE} text-orange-500`} />;
    }
    return <FileText className={`${ICON_SIZE} text-muted-foreground`} />;
  };
          ...
          <div key={category} className="bg-card rounded-xl border border-border shadow-lg">
            ...
            >
              <div className="flex items-center gap-3">
                <div className={`${CATEGORY_ICON_SIZE} bg-muted rounded-lg flex items-center justify-center`}>
                  <span className="text-lg">{getCategoryIcon(category)}</span>
                </div>
                <div className="flex-1 text-left">
                  <div className="font-medium text-lg">{category}</div>
                  <div className="text-xs text-muted-foreground">
                    {getFileCountText(categoryFiles.length)}
                  </div>
                </div>
              </div>
              <div className="flex items-center gap-2 z-10">
                ...
                {expandedCategories.has(category) ? (
                  <ChevronDown className={`${ICON_SIZE} text-muted-foreground`} />
                ) : (
                  <ChevronRight className={`${ICON_SIZE} text-muted-foreground`} />
                )}
              </div>
            </button>
            ...
            {expandedCategories.has(category) && (
              <div className="border-t border-border">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 p-4 lg:p-6">
                  {sortFilesByDate(categoryFiles).map((file) => (
                    <div
                      key={file.id}
